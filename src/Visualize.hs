...
plotF xd g = plot{ys = map (toFloat . g . fromFloat) (xvals xd),xs = xvals xd}
                  where xvals (a,b,d) | a > b = []
                                      | otherwise  = a:xvals (a+d,b,d)
...
figP fe ps = do let xl = sort $ nub $ concatMap xs ps
                ...
                let maxx = maximum xl
                let n = length xl
                let ys' = map (ys . yls xl) ps -- yls xl ps
                ...
                out1' ("plot(x,y,type=\"n\",main=\""++
                        ...
                        xLabel fe++"\",ylab=\""++
                        yLabel fe++"\")")
                mapM_ out1' ([drawy (n+1) (ps !! n) (ys' !! n) |
                                 n <- [0..length ys' - 1]])
                if all (null . label) ps then
                    return ()
                 else
