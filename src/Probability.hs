...
showR n x = replicate (n - length strX) ' ' ++ strX
  where
    strX = show x
...
instance Applicative Dist where
  pure x = D [(x, 1)]
  (D fs) <*> (D xs) = D [(f x, pf * px) | (f, pf) <- fs, (x, px) <- xs]
...
instance MonadPlus Dist where
  mzero      = D []
  mplus d d' = case (d, d') of
                 (D d1, D d2) | isZero d || isZero d' -> mzero
                              | otherwise             -> unfoldD $ choose 0.5 (D d1) (D d2)
...
-- Built by Neel!
-- Function to convert a Dist to a string
distToString :: Show a => Dist a -> String
distToString (D xs) = unlines $ map (\(x, p) -> show x ++ ": " ++ show p) xs

-- Built by Neel!
-- Function to convert a Spread to a string
spreadToString :: Show a => Spread a -> [a] -> String
spreadToString spread xs = distToString (spread xs)

-- Built by Neel!
-- Function to print a Spread
printSpread :: Show a => Spread a -> [a] -> IO ()
printSpread spread xs = putStrLn $ spreadToString spread xs
...
-- Updating standard normal to have a mean of 0 and a STD of 1
normal :: Spread a
normal = shape (normalCurve 0 1)

normalCurve :: Float -> Float -> Float -> Float
normalCurve mean stddev x = 1 / sqrt (2 * pi) * exp (-1/2 * u^2)
    where u = (x - mean) / stddev

lanczosCoef = [76.18009172947146, -86.50532032941677, 24.01409824083091, -1.231739572450155,
            0.001208650973866179, -0.000005395239384953]

-- Built by Neel!
-- Log-Gamma dist
logGamma :: Float -> Spread Float
logGamma x = \_ -> D [(logGammaVal x, 1)]
  where
    logGammaVal y = (y - 0.5) * log y - y + 0.5 * log (2 * pi) + sum (zipWith (/) lanczosCoef [1..])

-- Built by Neel!
-- Gamma dist
gamma :: Float -> Spread Float
gamma z = \_ -> D [(exp (logGammaVal z), 1)]
  where
    logGammaVal y = (y - 0.5) * log y - y + 0.5 * log (2 * pi) + sum (zipWith (/) lanczosCoef [1..])

-- Built by Neel!
-- Beta distribution density function
betaDensity :: Float -> Float -> Float -> Float
betaDensity alpha beta x = (x ** (alpha - 1)) * ((1 - x) ** (beta - 1)) / betaFunc alpha beta
  where
    betaFunc :: Float -> Float -> Float
    betaFunc a b = exp (logGammaVal a + logGammaVal b - logGammaVal (a + b))

    logGammaVal :: Float -> Float
    logGammaVal y = (y - 0.5) * log y - y + 0.5 * log (2 * pi) + sum (zipWith (/) lanczosCoef [1..])

-- Built by Neel!
-- Beta distribution
beta :: Float -> Float -> Spread Float
beta alpha beta = shape (betaDensity alpha beta)
